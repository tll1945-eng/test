...
int	maxset;
{
    ...
    int my_index;
    my_index = *i;
    while ((src[my_index] != delim) && (src[my_index] != ENDSTR)) 
    {
	my_index = *i - 1;
	if (src[my_index] == ESCAPE) {
	    escjunk = esc(src, i);
	    junk = addstr(escjunk, dest, j, maxset);
	} else	{
	    my_index = *i;
	    if (src[my_index] != DASH)
		junk = addstr(src[my_index], dest, j, maxset);
	    else if (*j <= 1 || src[my_index + 1] == ENDSTR)
		junk = addstr(DASH, dest, j, maxset);
	    else if ((isalnum(src[my_index - 1])) && (isalnum(src[my_index + 1])) 
		&& (src[my_index - 1] <= src[my_index + 1]))
		{
		    for (k = src[my_index-1]+1; k<=src[my_index+1]; k++) 
		    {
